...
import {Property, Location, NekretninaGetVM, SlikeGetVM} from './app.models';
...
export class Data {
  constructor(public properties: NekretninaGetVM[],
              public compareList: NekretninaGetVM[],
              public favorites: NekretninaGetVM[],
...
export class AppService {
  ...
  public getProperties(): Observable<NekretninaGetVM[]>{
    return this.http.get<NekretninaGetVM[]>(environment.url+'/Nekretnina/GetAll');
  }
  public getSlike(nekretninaId:any): Observable<SlikeGetVM[]>{
   return this.http.get<SlikeGetVM[]>(environment.url + '/Slike/GetAll?nekretninaID=' + nekretninaId);
  }

  public  getPropertyById(id): Observable<NekretninaGetVM>{
    //return this.http.get<NekretninaGetVM[]>(this.url + 'property-' + id + '.json');
    return this.http.get<NekretninaGetVM>(environment.url+'/Nekretnina/GetAll?id='+id);
  }

  public getFeaturedProperties(): Observable<NekretninaGetVM[]>{
    return this.http.get<NekretninaGetVM[]>(environment.url+'/Nekretnina/GetAll');
  ...
  public addToCompare(property:NekretninaGetVM, component, direction){
  ...
  public addToFavorites(property:NekretninaGetVM, direction){
    if(!this.Data.favorites.filter(item=>item.id == property.id)[0]){
      this.Data.favorites.push(property);
      this.snackBar.open('The property "' + property.naziv + '" has been added to favorites.', '×', {
  ...
  public getCities(){
    return [ 
      ...
      { id: 2, name: 'Sarajevo' },
      { id: 3, name: 'Mostar' },
      { id: 4, name: 'Sarajevo - Novi Grad' },
      { id: 5, name: 'Banja Luka' },
      { id: 6, name: 'Istočno Sarajevo' },
      { id: 7, name: 'Kiseljak' },
      { id: 8, name: 'Zenica' },
  ...
  public getFeatures(){
    return [ 
      { id: 1, name: 'Novo', selected: false },
      { id: 2, name: 'Korišteno', selected: false },
  ...
  public filterData(data: any, params: any, sort?: any, page?: any, perPage?: any){ 
    ...
    if(params){
      ...
      if(params.propertyType){
        data = data.filter(property => property.tip_nekretnine == params.propertyType.name)
      }
      ...
      if(params.propertyStatus && params.propertyStatus.length){       
        ...
        /*data.filter((property: any) =>
          property.tip_akcije.forEach((status: any) => {
        ...
        data.filter((property: any) => {
          if (statuses.indexOf(property.tip_akcije) > -1 && !properties.includes(property)) {
            properties.push(property);
          }
        });
        ...
        //if(this.appSettings.settings.currency == 'USD'){
          if(params.price.from){
            data = data.filter(property => {
              ...
              if(property.cijena && property.cijena >= params.price.from ){
                return true;
              }
          ...
          if(params.price.to){
            data = data.filter(property => {
              ...
              if(property.cijena && property.cijena <= params.price.to){
                return true;
              }
      ...
      if(params.city){
        data = data.filter(property => property.lokacija == params.city.name)
      }
      ...
      /*if (params.features) {
        ...
        if (arr.length > 0) {
          let properties: any[] = [];
          data.filter(property => {
            if (arr.includes(property.isKoristeno)) {
              if (!properties.includes(property)) {
                properties.push(property);
              }
            }
    ...
    if(sort){
      switch (sort) {
        ...
        case 'Popularni':
          ...
          break;
        case 'Cijena (od najniže)':
          //if(this.appSettings.settings.currency == 'USD'){
          data = data.filter(item => item.cijena !== "Na upit")
              .sort((a, b) => {
                const cijenaA = parseFloat(a.cijena.replace(/\.|,| KM/g, ''));
                const cijenaB = parseFloat(b.cijena.replace(/\.|,| KM/g, ''));
                if (cijenaA < cijenaB) {
                  return -1;
                }
                if (cijenaA > cijenaB) {
